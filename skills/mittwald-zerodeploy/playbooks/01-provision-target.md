...
**Goal**: Verify the user has a working mStudio project and CLI authentication before attempting deployment.
...
**If not installed**: Direct the user to https://developer.mittwald.de/docs/v2/cli/ for installation instructions.
...
Look for a line like `Project: p-xxxxxx (Project Name)`.
...
This avoids needing to pass `--project-id` on every deployment.
...
Once verification passes, proceed to **02-cli-deploy-local.md** to test deployment locally.
