...
- `MITTWALD_SSH_IDENTITY_FILE` / `MITTWALD_SSH_USER` (or `--ssh-identity-file` / `--ssh-user`) select the key and user for the SSH-tunnelling commands (`mw app exec`, `mw database mysql import`/`dump`; verified on `mw 1.19.0`) — for when the key isn't the default `~/.ssh/id_*`.
